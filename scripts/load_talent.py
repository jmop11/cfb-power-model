...
for year in range(START_YEAR, END_YEAR + 1):
    ...
    if not data:
        print(f"{year}: no talent data published yet, skipping")
        continue
    ...
    for entry in data:
        ...
        if cur.fetchone():
            cur.execute(
                "UPDATE team_season SET talent_composite = ? WHERE team_id = ? AND season = ?",
                (talent, team_id, year)
            )
            updated += 1
        else:
            cur.execute(
                "INSERT INTO team_season (team_id, season, talent_composite) VALUES (?, ?, ?)",
                (team_id, year, talent)
            )
            inserted += 1
